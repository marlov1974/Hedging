...
Baseloads does not create a separate customer hedge leg by default. Its hedge-relevant position is market basis exposure plus any configured customer add-ons.

Peaks/Modern customer calloffs use Modern customer basis:
...
Commercial add-ons are separate customer event details, not part of hedge price:
...
Baseloads is the market-near exception: the hedge-relevant position is stored in the market leg as `market.base.<area>`, while configured customer add-ons such as `fee.calloff` remain customer details.

Peaks/Modern calloffs create Modern customer hedge details, market basis details, and configured customer add-on details. Q-term and P-agent follow signed `modern.peak`; calloff fee is always positive and uses absolute customer-facing calloff volume.

## Rebalance/product change

Product change must not rewrite historical events.

Upgrade/downgrade creates forward-moving calloff events:
...
Baseloads-to-Modern upgrade is represented as two linked calloffs:

```text
MARKET_REBALANCE_CALLOFF    = trades only market base delta
CUSTOMER_CONVERSION_CALLOFF = creates Modern customer components and add-ons from resulting open market position
```

The customer conversion calloff derives customer hedge prices from effective open market base value divided by volume. It does not call the price API and does not create additional market delta.
